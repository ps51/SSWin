...
//   });
// });
//  function CreateLine(ids, w, c, div, bg) {//数组  width   颜色  控制器  背景颜色
//             var list = ids.split(","); 
           ...
           var sum = "";
            ...
            for (var i = 1; i < data.length; i++) {
                ...
                for (var j = 1; j < 13; j++) {
                    ...
                    tbody += "<td ><span class='colors' id='T" + i + "_" + j + "'>"+data.number+"</span></td>";
                }console.log(aas)
                tbody += "</tr>";
            }
            $("#zstable thead").html(head);
            ...
            ids = ids.substring(0, ids.length - 1);
            CreateLine(ids, "22", "#ff6600", "canvasdiv", "red"); 
             function CreateLine(ids, w, c, div, bg) {//数组  width   颜色  控制器  背景颜色
            var list = ids.split(",");
            console.log(list) 
